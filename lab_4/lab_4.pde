...
float circleTime = 6000;
float angle_increase = 0;
//Implementtion 1 line
//float angle_increase = TWO_PI / (baseFrameRate * 100);
...
public void SimulationThread(){
  while(1==1) {
      ...
      if(haplyBoard.data_available()){
        //implememtatino 2 lines
        float remainingTime = timesincelastloop % (circleTime * 1000000);
        float increase = remainingTime/(circleTime * 1000000);
        angle_increase = TWO_PI * increase;
        
        //move end effector in a circle, (xr,yr) center
        target_x = xr + cos(angle) * radius;
        target_y = yr + sin(angle) * radius; 
